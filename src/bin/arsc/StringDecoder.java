...
public class StringDecoder {
    ...
    public static StringDecoder read(ZInput mIn) throws IOException {
        ...
        int stringsOffset = block.stringsOffset = mIn.readInt();
        int stylesOffset = block.stylesOffset = mIn.readInt();

        block.m_isUTF8 = (flags & IS_UTF8) != 0;
        block.m_stringOffsets = mIn.readIntArray(stringCount);
        ...
        int i = 0;
        block.m_strings = new String[block.m_stringOffsets.length];
        for (int offset : block.m_stringOffsets) {
            int length;
            if (!block.m_isUTF8) {
    ...
    public void write(String[] s, ZOutput out) throws IOException {
		...
		/*
         * if (!this.m_isUTF8) { length = getShort(this.m_strings, offset) * 2;
		 * offset += 2; } else { offset += getVarint(this.m_strings, offset)[1];
		 * int[] varint = getVarint(this.m_strings, offset); offset +=
		 * varint[1]; length = varint[0]; }
		 */
        // m_strings
        ...
        led.writeInt(stylesOffset == 0 ? 0 : stylesOffset + i);
    ...
    private static byte[] getVarbytes(int val) {
