 ...
 * {@code bin.zip.ZipEntry} instead of
 ...
 *   {@code bin.zip.ZipEntry} instances.</li>
...
//        this.encoding = encoding;
    ...
    public Enumeration<ZipEntry> getEntriesInPhysicalOrder() {
        final ZipEntry[] allEntries = entries.toArray(new ZipEntry[entries.size()]);
    ...
    public InputStream getRawInputStream(final ZipEntry ze)
            throws IOException, ZipException {
        if (!(ze instanceof Entry)) {
            return null;
        }
        ...
        final BoundedInputStream bis =
                new BoundedInputStream(start, ze.getCompressedSize());
//        System.out.print(ze.getName()+" "+ze.getCompressedSize()+"\n");
        switch (ze.getMethod()) {
            case ZipEntry.STORED:
            case ZipEntry.DEFLATED:
                return bis;
