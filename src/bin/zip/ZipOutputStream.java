...
import bin.zip.exception.Zip64RequiredException;
import bin.zip.extrafiled.UnicodeCommentExtraField;
import bin.zip.extrafiled.UnicodePathExtraField;
import bin.zip.extrafiled.Zip64ExtendedInformationExtraField;
import bin.zip.util.GeneralPurposeBit;
import bin.zip.util.Zip64Mode;
import bin.zip.util.ZipEightByteInteger;
import bin.zip.util.ZipLong;
import bin.zip.util.ZipShort;
import bin.zip.util.ZipUtil;

import static bin.zip.util.ZipConstants.DATA_DESCRIPTOR_MIN_VERSION;
import static bin.zip.util.ZipConstants.DWORD;
import static bin.zip.util.ZipConstants.INITIAL_VERSION;
import static bin.zip.util.ZipConstants.SHORT;
import static bin.zip.util.ZipConstants.WORD;
import static bin.zip.util.ZipConstants.ZIP64_MAGIC;
import static bin.zip.util.ZipConstants.ZIP64_MAGIC_SHORT;
import static bin.zip.util.ZipConstants.ZIP64_MIN_VERSION;
import static bin.zip.util.ZipLong.putLong;
import static bin.zip.util.ZipShort.putShort;
...
public class ZipOutputStream extends FilterOutputStream {
   ...
   // CheckStyle:VisibilityModifier OFF - bc
    ...
    private void flushDeflater() throws IOException {
        if (entry.entry.getMethod() == DEFLATED && !currentIsRawEntry) {
            def.finish();
            while (!def.finished()) {
                deflate();
            }
    ...
    private boolean handleSizesAndCrc(long bytesWritten, long crc,
        ...
        if (entry.entry.getMethod() == DEFLATED) {
            ...
            */
            ...
            } else if (entry.entry.getCompressedSize() != bytesWritten) {
        ...
        } else if(!currentIsRawEntry) { /* method is STORED and we used RandomAccessFile */
    ...
    private boolean currentIsRawEntry = false;
    public void putNextRawEntry(ZipEntry archiveEntry) throws IOException {
        if (finished) {
            throw new IOException("Stream has already been finished");
        }
...
//        setDefaults(entry.entry);
    ...
    private byte[] createLocalFileHeader(ZipEntry ze, ByteBuffer name, boolean encodable)  {
        ...
        // CRC
        if (!currentIsRawEntry && (zipMethod == DEFLATED || raf != null)) {
        ...
        } else if (!currentIsRawEntry && (zipMethod == DEFLATED || raf != null)) {
